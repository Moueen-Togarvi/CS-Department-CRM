...
export interface BarProps {
  ...
  fill?: string;
  /** Row key holding a per-bar fill colour. Overrides `fill` when the row
   *  carries a string there — for ordinal categories that each need their own
   *  step of a ramp. Keep `fill` set as the fallback. */
  fillKey?: string;
  ...
  return (
    <g className={`bar-series-${uniqueId}`}>
      {data.map((d, i) => {
        ...
        const barFill =
          fillKey && typeof d[fillKey] === "string" ? (d[fillKey] as string) : fill;
