...
export function AppSidebar({ ...props }: React.ComponentProps<typeof Sidebar>) {
  ...
  return (
    <Sidebar variant="inset" {...props}>
      <SidebarHeader>
        <div className="flex items-center h-12 shrink-0 px-2 gap-2.5">
      ...
      <SidebarContent>
        {groupedItems.map((group) => (
          <SidebarGroup key={group.name} className="py-1">
            <SidebarGroupLabel className="text-zinc-500 font-semibold px-2">{group.name}</SidebarGroupLabel>
            <SidebarGroupContent>
                ...
                {group.items.map((item) => {
                  ...
                  return (
                    <SidebarMenuItem key={item.id}>
                      ...
                      >
                        <Icon className={cn("size-4", isActive ? "text-emerald-600" : "text-zinc-500")} />
        ...
        <SidebarMenu>
          <SidebarMenuItem>
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                ...
                >
                  <Avatar className="h-8 w-8 rounded-lg">
                    ...
                    <AvatarFallback className="rounded-lg bg-orange-600 text-white font-bold">{initials}</AvatarFallback>
                  </Avatar>
              ...
              >
                <DropdownMenuItem onClick={() => setActiveModule('profile')} className="cursor-pointer">
